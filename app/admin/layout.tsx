...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // أغلق السايدبار عند تغيير الصفحة
  ...
  if (!authenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 overflow-hidden relative"
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(20)].map((_, i) => (
            <div key={i} className="absolute rounded-full bg-white"
              style={{ width: Math.random() * 3 + 1 + "px", height: Math.random() * 3 + 1 + "px", top: Math.random() * 100 + "%", left: Math.random() * 100 + "%", opacity: Math.random() * 0.7 + 0.2, animation: `pulse ${Math.random() * 3 + 2}s ease-in-out infinite` }} />
          ))}
        </div>
        <div className="absolute inset-0 pointer-events-none select-none">
          <span className="absolute text-3xl opacity-20 animate-bounce" style={{ top: "10%", left: "8%", animationDuration: "3s" }}>🎨</span>
        ...
        <div className="absolute top-1/4 left-1/4 w-64 h-64 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(124,58,237,0.15) 0%, transparent 70%)" }} />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(59,130,246,0.12) 0%, transparent 70%)" }} />

        <div className="relative w-full max-w-sm z-10"
          style={{ background: "linear-gradient(145deg, rgba(255,255,255,0.07) 0%, rgba(255,255,255,0.03) 100%)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "28px", boxShadow: "0 32px 64px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.1)" }}>
          <div className="p-8">
            <div className="text-center mb-8">
              <div className="relative inline-block mb-4">
                <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto"
                  style={{ background: "linear-gradient(135deg, #f59e0b, #f97316)", boxShadow: "0 8px 24px rgba(245,158,11,0.4)", transform: "rotate(-3deg)" }}>
              ...
              <h1 className="text-3xl font-black mb-1" style={{ background: "linear-gradient(90deg, #f59e0b, #a78bfa, #60a5fa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", filter: "drop-shadow(0 2px 8px rgba(167,139,250,0.4))" }}>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className="block text-xs font-bold mb-2 tracking-wider uppercase" style={{ color: "rgba(196,181,253,0.8)" }}>كلمة المرور</label>
                ...
                {error && (
                  <div className="flex items-center gap-2 mt-2 px-3 py-2 rounded-xl" style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)" }}>
                    <span className="text-red-400 text-xs">{error}</span>
                  </div>
                )}
              </div>
              <button type="submit" disabled={loginLoading}
                className="w-full py-3.5 rounded-2xl font-black text-white text-sm transition-all disabled:opacity-50"
            ...
            <div className="mt-3 text-center">
              <button type="button" onClick={() => { setShowRecovery(!showRecovery); setRecoveryMsg(null); }}
                className="text-xs transition-colors" style={{ color: showRecovery ? "rgba(167,139,250,0.9)" : "rgba(255,255,255,0.25)" }}>
              ...
              <div className="mt-4 space-y-4" style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: "1rem" }}>
                <div className="flex rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
                  {(["key", "telegram"] as const).map((m) => (
                    <button key={m} type="button" onClick={() => { setRecoveryMethod(m); setRecoveryMsg(null); setOtpSent(false); }}
                      className="flex-1 py-2.5 text-xs font-semibold transition-all"
                      style={recoveryMethod === m ? { background: "linear-gradient(135deg,rgba(124,58,237,0.6),rgba(79,70,229,0.6))", color: "#fff" } : { color: "rgba(255,255,255,0.35)" }}>
                ...
                {recoveryMethod === "key" && (
                  <form onSubmit={handleRecovery} className="space-y-3">
                    <input type="text" value={recoveryKey} onChange={(e) => setRecoveryKey(e.target.value)} placeholder="مفتاح الاسترجاع" dir="ltr" autoComplete="off"
                      className="w-full px-4 py-3 rounded-2xl text-white placeholder-gray-600 outline-none text-sm"
                      style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)" }} />
                    <input type="password" value={newPass} onChange={(e) => setNewPass(e.target.value)} placeholder="كلمة السر الجديدة"
                      className="w-full px-4 py-3 rounded-2xl text-white placeholder-gray-600 outline-none text-sm"
                      style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)" }} />
                ...
                {recoveryMethod === "telegram" && (
                  <div className="space-y-3">
                    {!otpSent ? (
                      ...
                      <form onSubmit={handleRecovery} className="space-y-3">
                        <p className="text-xs text-center" style={{ color: "rgba(255,255,255,0.4)" }}>أدخل الكود المكون من 6 أرقام</p>
            ...
            <p className="text-center text-xs mt-6" style={{ color: "rgba(255,255,255,0.2)" }}>عالم الأطفال © {new Date().getFullYear()}</p>
          </div>
        </div>
      </div>
    );
  ...
  const activeGradient = activeLink ? (navColors[activeLink.href] ?? "from-violet-500 to-purple-600") : "from-violet-500 to-purple-600";

  const bg      = isDark ? "#0d0f18" : "#f1f3f9";
  const bgHdr   = isDark ? "rgba(13,15,24,0.97)" : "rgba(255,255,255,0.97)";
  const borderC = isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.07)";
  const textC   = isDark ? "#f1f5f9" : "#1f2937";

  return (
    <div className="min-h-screen" dir="rtl" style={{ background: "#0f1117" }}>

      {/* ══ Sidebar ══
          الهاتف  : translate-x-full (مخفي) ← sidebarOpen يتحكم فيه
          ديسكتوب : translate-x-0 دائماً (lg:translate-x-0)
      */}
      <aside
      ...
      >
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
          <p className="text-xs font-bold px-3 mb-3 tracking-widest uppercase" style={{ color: "rgba(255,255,255,0.2)" }}>القائمة</p>
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} onClick={() => setSidebarOpen(false)}
                ...
                style={isActive
                  ? { background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 4px 16px rgba(0,0,0,0.2)", color: "#fff" }
                  : { color: "rgba(255,255,255,0.4)", border: "1px solid transparent" }}>
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-gradient-to-br ${gradient}`}
                  style={{ opacity: isActive ? 1 : 0.5, boxShadow: isActive ? "0 4px 12px rgba(0,0,0,0.3)" : "none" }}>
                  <link.icon className="w-4 h-4 text-white" />
                </div>
                <span>{link.label}</span>
                {isActive && <div className="mr-auto w-1.5 h-1.5 rounded-full bg-white" style={{ boxShadow: "0 0 6px #fff" }} />}
              </Link>
            );
          })}
        </nav>
      ...
      {/* ══ المحتوى الرئيسي ══
          lg:mr-64 → يترك مكان السايدبار على الديسكتوب
          على الهاتف: mr-0 (بدون هامش) لأن السايدبار يطفو فوق المحتوى
      */}
      <div className="flex flex-col min-h-screen lg:mr-64" style={{ background: bg }}>
        ...
        >
          {/*
            ✅ الحل الصحيح: استخدام CSS class بدل JS
            - lg:hidden  → CSS يخفيه على الديسكتوب
            - على الهاتف → يظهر دائماً بدون أي JS أو state
            - في RTL: أول عنصر في flex = يمين الشاشة ✓
          */}
          <button
            onClick={() => setSidebarOpen((v) => !v)}
            aria-label="القائمة"
            className="flex-shrink-0 w-11 h-11 flex items-center justify-center rounded-xl transition-all active:scale-95 lg:hidden"
            style={{
          ...
          <div className="flex-1 min-w-0 flex items-center gap-2">
            ...
            <h1 className="font-black text-base truncate" style={{ color: textC }}>{activeLabel}</h1>
          </div>

          {/* الوضع الليلي + رابط الموقع */}
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={toggleDark}
              className="w-9 h-9 flex items-center justify-center rounded-xl transition-all"
              style={{ background: isDark ? "rgba(251,191,36,0.15)" : "rgba(0,0,0,0.04)", border: isDark ? "1px solid rgba(251,191,36,0.25)" : "1px solid rgba(0,0,0,0.06)" }}>
